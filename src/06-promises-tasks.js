...
async function chainPromises(array, action) {
  async function getResolve() {
    return array.reduce((priv, el) => {
      el.then((value) => priv.push(Promise.resolve(value))).catch((err) => err);
      return priv;
    }, []);
  }
  const privArr = await getResolve();

  return privArr.reduce((priv, el) => priv.then((val) => el.then((val2) => action(val, val2))));
}
